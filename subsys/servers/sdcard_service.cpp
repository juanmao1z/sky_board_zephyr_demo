...
int SdcardService::run() noexcept {
  ...
  for (int attempt = 1; attempt <= k_max_attempts; ++attempt) {
    ...
    if (attempt < k_max_attempts) {
      ...
      char msg[96] = {};
      (void)snprintf(msg, sizeof(msg), "[sd] retry %d/%d after err=%d", attempt, k_max_attempts,
                     last_err);
      log_.info(msg);
 ...
 * @return 0 表示成功；负值表示失败。
 */
int SdcardService::write_file(const char* path, const void* data, size_t len,
 ...
 * @return 0 表示成功；负值表示失败。
 */
int SdcardService::read_file(const char* path, void* buffer, size_t buffer_size,
 ...
 */
int SdcardService::enqueue_write(const char* path, const void* data, size_t len,
                                 bool append) noexcept {
  (void)path;
  (void)data;
  (void)len;
