...
void TimeService::maybe_probe_beijing_time_once() noexcept {
  ...
  if (ret < 0) {
    if (probe_attempt_count_ >= kMaxProbeAttempts) {
      ...
      char msg[96];
      (void)snprintf(msg, sizeof(msg), "[time] Beijing sync failed: err=%d, retry in 10s (%u/%u)",
                     ret, static_cast<unsigned int>(probe_attempt_count_),
                     static_cast<unsigned int>(kMaxProbeAttempts));
      log_.info(msg);
    }
    return;
...
int TimeService::fetch_utc_epoch_from_http_date(time_t& out_epoch_sec) noexcept {
  ...
  if (zsock_connect(sock, reinterpret_cast<struct sockaddr*>(&server_addr), sizeof(server_addr)) <
      0) {
    ret = -errno;
    close_fd(sock);
    return ret;
  ...
  while (send_total < (sizeof(request) - 1U)) {
    const ssize_t sent =
        zsock_send(sock, request + send_total, (sizeof(request) - 1U) - send_total, 0);
    if (sent < 0) {
      ret = -errno;
      close_fd(sock);
      return ret;
  ...
  while (recv_total < (sizeof(headers) - 1U)) {
    const ssize_t r =
        zsock_recv(sock, headers + recv_total, (sizeof(headers) - 1U) - recv_total, 0);
    if (r == 0) {
      break;
    }
 ...
 * @return 0 表示成功；负值表示失败。
 */
int TimeService::extract_http_date_header(const char* headers, char* out_date,
 ...
 * @return 0 表示成功；负值表示失败。
 */
int TimeService::parse_rfc7231_date_to_epoch(const char* date_value,
                                             time_t& out_epoch_sec) const noexcept {
  /*
   * 执行步骤：
  ...
  const int matched = sscanf(date_value, "%3[^,], %d %3s %d %d:%d:%d %3s", weekday, &day, month,
                             &year, &hour, &minute, &second, zone);
  if (matched != 8) {
    return -EINVAL;
  }
