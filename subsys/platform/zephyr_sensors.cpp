...
public:
	/** @brief 返回该驱动绑定的传感器类型。 */
	platform::SensorType type() const noexcept override
	{
		return platform::SensorType::Ina226;
	}
	...
	/** @brief 返回该驱动样本结构大小。 */
	size_t sample_size() const noexcept override
	{
		return sizeof(platform::Ina226Sample);
	}
...
public:
	...
	platform::SensorType type() const noexcept override
	{
		return platform::SensorType::Aht20;
	}
	...
	size_t sample_size() const noexcept override
	{
		return sizeof(platform::Aht20Sample);
	}
 ...
 * @return 当前驱动数量。
 */
size_t SensorHub::registered_count() const noexcept
{
	return driver_count_;
}
...
{
	return sensor_hub().read_ina226_once(out);
}
...
{
	return sensor_hub().read_aht20_once(out);
}
