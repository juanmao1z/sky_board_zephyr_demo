...
public:
	 ...
	 * @return 0 成功；负值失败。
	 */
	int set_enabled(bool on) noexcept override
	{
		return set_brightness(on ? 100U : 0U);
	}
	...
	int set_brightness(uint8_t percent) noexcept override
	{
		...
		static const struct pwm_dt_spec backlight_pwm =
			PWM_DT_SPEC_GET(BACKLIGHT_PWM_NODE);
		...
		const uint32_t pulse =
			(backlight_pwm.period * static_cast<uint32_t>(percent)) / 100U;
