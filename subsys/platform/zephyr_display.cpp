...
private:
	 ...
	 * @return 0 成功；负值失败。
	 */
	int write_solid_rect(uint16_t x,
			     uint16_t y,
			     uint16_t w,
			     uint16_t h,
 ...
 * @return 0 成功；负值失败。
 */
int ZephyrDisplay::write_solid_rect(uint16_t x,
				    uint16_t y,
				    uint16_t w,
				    uint16_t h,
				    uint16_t color_rgb565) noexcept
 ...
 * @return 0 成功；负值失败。
 */
int ZephyrDisplay::fill_rect(uint16_t x,
			     uint16_t y,
			     uint16_t w,
			     uint16_t h,
			     uint16_t color_rgb565) noexcept
 ...
 * @return 0 成功；负值失败。
 */
int ZephyrDisplay::draw_char(uint16_t x,
			     uint16_t y,
			     char c,
			     uint16_t fg_rgb565,
			     uint16_t bg_rgb565,
			     uint8_t scale) noexcept
{
	...
	for (uint8_t col = 0U; col < platform::font5x7::kWidth; ++col) {
		...
		for (uint8_t row = 0U; row < platform::font5x7::kHeight; ++row) {
			...
			ret = write_solid_rect(static_cast<uint16_t>(px),
 ...
 * @return 0 成功；负值失败。
 */
int ZephyrDisplay::draw_text(uint16_t x,
			     uint16_t y,
			     const char *text,
			     uint16_t fg_rgb565,
			     uint16_t bg_rgb565,
			     uint8_t scale) noexcept
{
	...
	uint16_t cursor_x = x;
	uint16_t cursor_y = y;
	const uint16_t step_x = static_cast<uint16_t>(
		(platform::font5x7::kWidth + platform::font5x7::kSpacing) * scale);
	const uint16_t step_y = static_cast<uint16_t>(
		(platform::font5x7::kHeight + platform::font5x7::kSpacing) * scale);
 ...
 * @return 0 成功；负值失败。
 */
int ZephyrDisplay::draw_int(uint16_t x,
			    uint16_t y,
			    int32_t value,
			    uint16_t fg_rgb565,
			    uint16_t bg_rgb565,
